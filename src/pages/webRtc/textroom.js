              ...
              ondata: function (data) {
                ...
                if (what === "message") {
                  ...
                  if (whisper === true) {
                    // Private message
                    $("#chatroom").append(
                      '<p style="color: purple;">[' + dateString + "] <b>[whisper from " + sender + "]</b> " + msg
                    );
                  ...
                  if (username !== myid && $("#rp" + username).length === 0) {
                    ...
                    $("#list").append(
                      '<li id="rp' + username + '" class="list-group-item">' + participants[username] + "</li>"
                    );
                    $("#rp" + username)
                      .css("cursor", "pointer")
                      .click(function () {
                  ...
                  $("#rp" + username).remove();
                  $("#chatroom").append(
                    '<p style="color: green;">[' + getDateString() + "] <i>" + participants[username] + " left</i></p>"
                  );
                  $("#chatroom").get(0).scrollTop = $("#chatroom").get(0).scrollHeight;
...
function registerUsername() {
  if ($("#username").length === 0) {
    ...
    if (username === "") {
      $("#you").removeClass().addClass("badge bg-warning").html("Insert your display name (e.g., pippo)");
      ...
      if (response.participants && response.participants.length > 0) {
        for (let i in response.participants) {
          ...
          if (p.username !== myid && $("#rp" + p.username).length === 0) {
            // Add to the participants list
            $("#list").append(
              '<li id="rp' + p.username + '" class="list-group-item">' + participants[p.username] + "</li>"
            );
            $("#rp" + p.username)
              .css("cursor", "pointer")
              .click(function () {
