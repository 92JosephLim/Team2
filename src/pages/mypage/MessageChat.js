    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-6">
          <section>
            <h2 className="text-4xl font-bold my-12">1:1 채팅 목록</h2>
            <ul className="bg-white shadow-md rounded-lg p-4 mb-36">
              {chatData.oneToOneChats.map((chat) => (
                <li key={chat.id} className="flex justify-between items-center mb-2 p-2 border-b  text-2xl">
                  <span className="block font-bold">{chat.name}</span>
                  <span className="block text-gray-600 text-center flex-1">{chat.lastMessage}</span>
                  <span className="text-gray-500 text-right">{chat.time}</span>
                </li>
              ))}
            </ul>
          </section>
          <section>
            <h2 className="text-4xl font-bold my-12">단체 채팅방 목록</h2>
